...
6.  **Downtime, Break, and Availability Rules:**
    ...
    *   **Break Time (MANDATORY & DYNAMIC):** There is a mandatory 30-minute break for all machines. This break MUST be scheduled exactly in the middle of the provided 'productionShiftDuration'. First, calculate the midpoint of the shift (e.g., \`productionShiftDuration / 2\`). Then, create a 30-minute unavailability window centered on that midpoint (from midpoint - 15 minutes to midpoint + 15 minutes). For example, if the shift is 540 minutes, the midpoint is 270, and the break is from minute 255 to 285. No operations can be scheduled during this 30-minute window for any machine. You must use the provided \`productionShiftDuration\` for this calculation, not a hardcoded value.
